...
async function handleRedirect(req, res) {
  try {
    ...
    const [rows] = await pool.query('SELECT id, original_url AS originalUrl, expires_at FROM links WHERE short_code = ? OR custom_alias = ?', [code, code]);
    ...
    // increment count
    await pool.query('UPDATE links SET click_count = click_count + 1 WHERE id = ?', [link.id]);
